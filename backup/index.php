...
<?php
if(isset($_GET['del'])){
    if(!isset($_GET['name'])){$_GET['name']="";}else{$_GET['name']="?name=".$_GET['name'];}
    ?>
<p class="status status--ERROR">
    <?php if(unlink($_GET['del'])==true){ ?>
  File deleted. <a href="<?php echo 'http://'.$_SERVER['SERVER_NAME'].substr($_SERVER['REQUEST_URI'],0,strpos($_SERVER['REQUEST_URI'], "?")).$_GET['name']; ?>">Restart</a>
    <?php }else{ ?>
...
// if (isset($_POST['dozip'])) {
  ...
  // Resulting zipfile e.g. zipper--2016-07-23--11-55.zip.
    if(isset($_GET['name'])){
        $zipfile = $_GET['name'] . '.zip';
    }else{
  $zipfile = 'site-' . date("d-m-Y--H-i") . '.zip';
    }
...
class Zipper {
  ...
  public static function zipDir($sourcePath, $outZipPath) {
    ...
    $GLOBALS['status'] = array('success' => 'Successfully created the backup file, downloading <a target="_blank" href="http://'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'].$outZipPath.'">'. $outZipPath .'</a>. <small><small><a href="http://'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'].'?del='.$outZipPath.$_GET['name'].'">DELETE</a></small></small>');
    
    //header("refresh:5;url=$outZipPath");
    //if(file_put_contents($outZipPath,file_get_contents('http://'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'].$outZipPath))){};
    echo '
    <script>
        setTimeout(function(){
            window.open("'.$outZipPath.'","_blank")
        }, 250);
        setTimeout(function(){
            window.location.href = "?del='.$outZipPath.$_GET['name'].'";
        }, 10000);
    </script>
    ';
